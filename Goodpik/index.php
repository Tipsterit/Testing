<!DOCTYPE html>

<html class="no-js" lang="">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="x-ua-compatible" content="ie=edge">
        <title>GoodPik | Free vector , PSD , Logos Download</title>
        <meta name="description" content="One of the best Image Download ">
        <meta name="viewport" content="width=device-width, initial-scale=1">

        <!-- Favicon
		============================================ -->
		<link rel="shortcut icon" type="image/x-icon" href="img/favicon.ico">        

 		<!-- CSS  -->
		
		<!-- Google Fonts CSS
		============================================ -->
		<link href='https://fonts.googleapis.com/css?family=Roboto:400,300,500,700' rel='stylesheet' type='text/css'>
		
		<!-- jquery-ui CSS
		============================================ -->		
        <link rel="stylesheet" href="css/jquery-ui.css">
		
		<!-- Bootstrap CSS
		============================================ -->      
        <link rel="stylesheet" href="css/bootstrap.min.css">	
        
		<!-- owl.carousel CSS
		============================================ -->      
        <link rel="stylesheet" href="css/owl.carousel.css">
        
		<!-- owl.theme CSS
		============================================ -->      
        <link rel="stylesheet" href="css/owl.theme.css">
           	
		<!-- owl.transitions CSS
		============================================ -->      
        <link rel="stylesheet" href="css/owl.transitions.css">
        
		<!-- font-awesome.min CSS
		============================================ -->      
        <link rel="stylesheet" href="css/font-awesome.min.css">
        
		<!-- nivo-slider css 
		============================================ -->
		<link rel="stylesheet" href="css/nivo-slider.css">
        
        <!-- fancybox CSS
		============================================ -->		
		<link rel="stylesheet" href="css/fancybox/jquery.fancybox.css">
        
 		<!-- animate CSS
		============================================ -->         
        <link rel="stylesheet" href="css/animate.css">
        
 		<!-- normalize CSS
		============================================ -->        
        <link rel="stylesheet" href="css/normalize.css">
        
        <!--Responsive Mobile Menu
        ============================================ --> 
        <link rel="stylesheet" href="css/meanmenu.min.css" />
   
        <!-- main CSS
		============================================ -->          
        <link rel="stylesheet" href="css/main.css">
        
        <!-- style CSS
		============================================ -->          
        <link rel="stylesheet" href="style.css">
        
        <!-- responsive CSS
		============================================ -->          
        <link rel="stylesheet" href="css/responsive.css">
        
        <script src="js/vendor/modernizr-2.8.3.min.js"></script>
        
    </head>
    <body class="home-2">
        
    <section class="header-area">
    
    <div class="header-middle">
        <div class="container-fluid">
            <div class="row">
               
                <div class="col-xs-6 col-sm-6 col-md-6 col-lg-7 no-padding">
                   <!--Logo start-->
                    <div class="logo">
                        <a href="index.php"><img src="img/good pic logo 2.png" alt="logo" style="background-color:#fff;height:60px;width:100px;padding:10px"></a>
                    </div>
                    <!--MAIN MENU START-->
                   <div class="main-menu hidden-sm hidden-xs hidden-md">
                            <nav>
                                <ul>
                                   
                                    <li><a href="#">Premium</a>
                                       <!-- <div class="mega-menu">
                                            <div class="mega-catagory">
                                                <h4><a href="#">Shop Layout</a></h4>
                                                <a href="shop.html">shop grid view</a>
                                                <a href="shop-list.html">shop List View</a>
                                                <a href="shop-box.html">shop grid view box</a>
                                                <a href="shop-list-box.html">shop List View box</a>
                                            </div>
                                            <div class="mega-catagory">
                                                <h4><a href="#">Shop Pages</a></h4>
                                                <a href="product.html">product page</a>
                                                <a href="cart.html">Shopping Cart</a>
                                                <a href="checkout.html">Checkout</a>
                                                <a href="product-box.html">product page box</a>
                                                <a href="cart-box.html">Shopping Cart box</a>
                                                <a href="checkout-box.html">Checkout box</a>
                                               
                                            </div>
                                            <div class="mega-catagory">
                                                <h4><a href="product.html">Product types</a></h4>
                                                <a href="product.html">simple product</a>
                                                <a href="product.html">variable product</a>
                                                <a href="product.html">grouped product</a>
                                                <a href="product.html">external product</a>
                                                <a href="product.html">downloadable</a>
                                                <a href="product.html">virtual product</a>
                                            </div>
                                        </div>  -->
                                    </li>
                                    <li><a href="#">Vectors</a>
                                      <!--  <div class="mega-menu ">
                                            <div class="mega-catagory">
                                                <h4><a href="#">Blog Full Width</a></h4>
                                                <a href="blog.html">non sidebar</a>
                                                <a href="blog-left-sidebar.html">Sidebar Left</a>
                                                <a href="blog-right-sidebar.html">Sidebar Right</a>
                                            </div>
                                            <div class="mega-catagory">
                                                <h4><a href="#">Blog Box Layout</a></h4>
                                                <a href="blog-box.html">non sidebar box</a>
                                                <a href="blog-left-sidebar-box.html">Sidebar Left box</a>
                                                <a href="blog-right-sidebar-box.html">Sidebar Right box</a>
                                            </div>
                                            <div class="mega-catagory">
                                                <h4><a href="#">Post Format</a></h4>
                                                <a href="single-post.html">image Product</a>
                                                <a href="single-post-video.html">video Product</a>
                                                <a href="single-post-gallery.html">gallery Product</a>
                                                <a href="single-post-audio.html">audio Product</a>
                                            </div>
                                        </div>  -->
                                    </li>
                                    <li><a href="#">PSD Files</a>
                                      <!--  <ul class="sub-menu">
                                            <li><a href="about.html">About us</a></li>
                                            <li><a href="about-box.html">About us box</a></li>
                                        </ul> -->
                                    </li>
                                    <li><a href="#">Icons</a>
                                       <!-- <ul class="sub-menu">
                                            <li><a href="portfolio-box.html">Portfolio box</a>
                                            <li><a href="portfolio-three-col.html">three columns</a></li>
                                            <li><a href="single-project.html">single project</a></li>  
                                            <li><a href="portfolio-three-col-box.html">three columns box</a></li>
                                            <li><a href="single-project-box.html">single project box</a></li> 
                                        </ul> -->
                                    </li>
                                    <li><a href="#">Stock Photos</a>
                                      <!--  <ul class="sub-menu">
                                            <li><a href="Contact-box.html">contact us box</a></li>   
                                        </ul> -->
                                    </li>
                                    
                                </ul>
				            </nav>
                        </div>
                    <!--MAIN MENU END-->
                </div>
                <div class="col-xs-6 col-sm-6 col-md-6 col-lg-3 no-padding-left">
                    <div class="top-right-menu-wrapper">
                        <div class="lang-currency">
                            <ul>
                                <!--SINGLE MENU WITH DROPDOWN-->
                                <li class="dropdown">
                                    <a data-toggle="dropdown" class="dropdown-toggle" href="#">
                                    <span>Language</span> <i class="fa fa-angle-down"></i></a>
                                    <ul class="dropdown-menu currency">
                                        <li><a href="#">english</a></li>
                                        <li><a href="#">Hindi</a></li>
										<li><a href="#">Gujarati</a></li>
                                    </ul>
                                </li>
                                <!--SINGLE MENU WITH DROPDOWN-->
                                
                            </ul>
                        </div>
                        <div class="plus-account">
                            <div class="plus-icon">
                                <i class="fa fa-plus"></i>
                            </div>
                            <div class="plus-menu">
                                <ul>
                                    <li><a href="#">contact</a></li>
                                    <li><a href="#">my account</a></li>
                                    <li><a href="#">wishlist</a></li>
                                    <li><a href="#">shopping cart</a></li>
                                    <li><a href="checkout.Php">checkout</a></li>
                                </ul>    
                            </div>
                        </div>
                        <div class="cart-wrapper">
                            <div class="cart">
                               <i class="fa fa-shopping-cart"></i>
                            </div>
                            <div class="cart-drop">
                                <div class="single-cart">
                                    <div class="cart-img">
                                       <img src="img/cart-drop.jpg" alt="">
                                       <span>2</span>
                                    </div>
                                    <div class="cart-title">
                                        <p><a href="#">aeniuyt estuc</a></p>
                                    </div>
                                    <div class="cart-price">
                                       <p>$<span>500</span></p> 
                                    </div>
                                    <a href="#"><i class="fa fa-times"></i></a>
                                </div>
                                <div class="cart-sub-total">
                                    <p>sub total <span>$550</span></p>
                                </div>
                                <div class="cart-checkout">
                                    <a href="checkout.html">checkout</a>
                                </div>
                            </div>
                        </div>
                        <div class="search-two">
                            <div class="search-icon-two">
                                <i class="fa fa-search"></i>
                            </div>
                            <div class="search-drop">
                               <div class="categorys-product-search">
                                    <form action="#" method="get" class="search-form-cat">
                                        <div class="search-product form-group">
                                            <select name="catsearch" class="cat-search">
                                                <option value="">All Categories</option>
                                                <option value="2">--Women</option>
                                                <option value="3">---T-Shirts</option>
                                                <option value="4">--Men</option>
                                                <option value="5">----Shoose</option>
                                                <option value="6">--Dress</option>
                                                <option value="7">----Tops</option>
                                                <option value="8">---Casual</option>
                                                <option value="9">--Evening</option>
                                                <option value="10">--Summer</option>
                                                <option value="11">---sports</option>
                                            </select>
                                            <input type="text" class="form-control search-form" placeholder="Enter your search key. "/>
                                            <button class="search-button" value="Search" name="s" type="submit">
                                                <i class="fa fa-search"></i>
                                            </button>									 
                                        </div>
                                    </form>
                                </div> 
                            </div>
                        </div>
                    </div>
                </div>
            </div> 
        </div>
    </div>
    <!--MOBILE MENU START-->
    <div class="mobile-menu ">
        <nav class="mobile-menu-start">
                                <ul>
                                    <li><a href="#">Home</a>
                                     <!--   <ul class="sub-menu">
                                            <li><a href="index-box.html">Homepage 1 Box</a></li>
                                            <li><a href="index-2.html">Homepage 2</a></li>
                                            <li><a href="index-2-box.html">Homepage 2 box</a></li>
                                        </ul>  -->
                                    </li>
                                    <li><a href="#">Premium</a>
                                       <!-- <div class="mega-menu">
                                            <div class="mega-catagory">
                                                <h4><a href="#">Shop Layout</a></h4>
                                                <a href="shop.html">shop grid view</a>
                                                <a href="shop-list.html">shop List View</a>
                                                <a href="shop-box.html">shop grid view box</a>
                                                <a href="shop-list-box.html">shop List View box</a>
                                            </div>
                                            <div class="mega-catagory">
                                                <h4><a href="#">Shop Pages</a></h4>
                                                <a href="product.html">product page</a>
                                                <a href="cart.html">Shopping Cart</a>
                                                <a href="checkout.html">Checkout</a>
                                                <a href="product-box.html">product page box</a>
                                                <a href="cart-box.html">Shopping Cart box</a>
                                                <a href="checkout-box.html">Checkout box</a>
                                               
                                            </div>
                                            <div class="mega-catagory">
                                                <h4><a href="product.html">Product types</a></h4>
                                                <a href="product.html">simple product</a>
                                                <a href="product.html">variable product</a>
                                                <a href="product.html">grouped product</a>
                                                <a href="product.html">external product</a>
                                                <a href="product.html">downloadable</a>
                                                <a href="product.html">virtual product</a>
                                            </div>
                                        </div>  -->
                                    </li>
                                    <li><a href="#">Vectors</a>
                                      <!--  <div class="mega-menu ">
                                            <div class="mega-catagory">
                                                <h4><a href="#">Blog Full Width</a></h4>
                                                <a href="blog.html">non sidebar</a>
                                                <a href="blog-left-sidebar.html">Sidebar Left</a>
                                                <a href="blog-right-sidebar.html">Sidebar Right</a>
                                            </div>
                                            <div class="mega-catagory">
                                                <h4><a href="#">Blog Box Layout</a></h4>
                                                <a href="blog-box.html">non sidebar box</a>
                                                <a href="blog-left-sidebar-box.html">Sidebar Left box</a>
                                                <a href="blog-right-sidebar-box.html">Sidebar Right box</a>
                                            </div>
                                            <div class="mega-catagory">
                                                <h4><a href="#">Post Format</a></h4>
                                                <a href="single-post.html">image Product</a>
                                                <a href="single-post-video.html">video Product</a>
                                                <a href="single-post-gallery.html">gallery Product</a>
                                                <a href="single-post-audio.html">audio Product</a>
                                            </div>
                                        </div>  -->
                                    </li>
                                    <li><a href="#">PSD Files</a>
                                      <!--  <ul class="sub-menu">
                                            <li><a href="about.html">About us</a></li>
                                            <li><a href="about-box.html">About us box</a></li>
                                        </ul> -->
                                    </li>
                                    <li><a href="#">Icons</a>
                                       <!-- <ul class="sub-menu">
                                            <li><a href="portfolio-box.html">Portfolio box</a>
                                            <li><a href="portfolio-three-col.html">three columns</a></li>
                                            <li><a href="single-project.html">single project</a></li>  
                                            <li><a href="portfolio-three-col-box.html">three columns box</a></li>
                                            <li><a href="single-project-box.html">single project box</a></li> 
                                        </ul> -->
                                    </li>
                                    <li><a href="#">Stock Photos</a>
                                      <!--  <ul class="sub-menu">
                                            <li><a href="Contact-box.html">contact us box</a></li>   
                                        </ul> -->
                                    </li>
                                    
                                </ul>
				            </nav>
    </div>
    <!--MOBILE MENU END-->        
    </section>
    <!--HEADER AREA END-->
    <div class="clear"></div><!--CLEAR BOTH CLASS-->
    <!--SLIDER AREA START-->
    <!-- <div class="slider-index-two">
        <div class="container-fluid">
            <div class="row">
				<!-- slider start -->
			<!--	<div class="slider">
					<div id="mainSlider" class="nivoSlider">
						<img src="img/slider-3.jpg" alt="main slider" title="#htmlcaption1"/>
						<img src="img/slider-4.jpg" alt="main slider" title="#htmlcaption2"/>
					</div>	
					<div id="htmlcaption1" class="nivo-html-caption slider-caption">
						<div class="slider-progress"></div>
						<div class="slider-text">
							<div class="middle-text">
								<div class="width-cap">
									<h3 class="slider-tiile-top top-ani-1"><span>Sale up to 50% off in august</span></h3>
									<h2 class="slider-tiile-middle middle-ani-1"><span class="bg-text">women</span></h2>
									<h2 class="slider-tiile-middle-2 middle-ani-2"><span>bags</span></h2>
								</div>
							</div>
						</div>
					</div>
					<div id="htmlcaption2" class="nivo-html-caption slider-caption">
						<div class="slider-progress"></div>
						<div class="slider-text-2 slider-text">
							<div class="middle-text">
								<div class="width-cap">
									<h3 class="slider-tiile-top top-ani-1"><span>Sale up to 50% off in august</span></h3>
									<h2 class="slider-tiile-middle middle-ani-1"><span class="bg-text">Iphone 6s</span></h2><br>
									<h2 class="slider-tiile-middle-2 middle-ani-2"><span>plus</span></h2>
									
								</div>
							</div>
						</div>					
					</div>
				</div>  -->
				<!-- slider end -->	
            </div>
        </div>
    </div>
    <!--SLIDER AREA END-->
    <div class="clear"></div><!--CLEAR BOTH CLASS-->
    <!-- BANNER AREA START-->
    <div class="banner-top-two">
        <div class="container-fluid">
            <div class="row">
               <!--SINGLE BANNER-->
                <div class="col-sm-6 col-md-4">
                    <div class="single-banner">
                        <div class="overlay"></div>
                        <p>
                            <a href="#"><img src="img/images (4).jpg" alt="" style="height:280px"></a>
                        </p>
                    </div>
                </div>
                <!--SINGLE BANNER-->
                <div class="col-sm-6 col-md-4">
                    <div class="single-banner">
                        <div class="overlay"></div>
                        <p>
                            <a href="#"><img src="img/images (9).jpg" alt="" style="height:280px"></a>
                        </p>
                    </div>
                </div>
                <!--SINGLE BANNER-->
                <div class="col-md-4 hidden-sm">
                    <div class="single-banner">
                        <div class="overlay"></div>
                        <p>
                            <a href="#"><img src="img/images (1).jpg" alt="" style="height:280px"></a>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- BANNER AREA END-->
    <!--CLIENT AREA START-->
    <div class="client-area">
        <div class="container-fluid">
            <div class="row">
                <div class="client-carousel">
                    <div class="col-md-12 width-hun">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-1.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-2.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 width-hun">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-3.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-4.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 width-hun">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-5.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-6.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 width-hun">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-7.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-8.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 width-hun">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-9.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-10.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 width-hun">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-11.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-12.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 width-hun">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-13.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-5.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 width-hun">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-7.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-4.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 width-hun">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-9.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-6.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 width-hun">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-11.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-8.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-1.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-2.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-3.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-4.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-1.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-2.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-3.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-4.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-5.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-6.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-7.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-8.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-9.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-10.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-11.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-12.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-13.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-5.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-7.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-4.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12 ">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-9.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-6.jpg" alt=""></a>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-11.jpg" alt=""></a>
                        </div>
                        <div class="client-img">
                            <a href="#"><img src="img/clients/client-8.jpg" alt=""></a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!--CLIENT AREA END-->
    <!--TAB AREA START-->
    <section class="tab-and-banner-wrapper">
        <div class="container-fluid">
            <!--START TOP TAB AREA -->
            <div class="tab-wrapper">
                <!--Start tab button -->
               <!-- <div class="row">
                    <div class="col-md-12">
                        <ul class="tab-button area-heading" role="tablist">
                            <li role="presentation" class="active">
                            <a href="#laptop" aria-controls="laptop" role="tab" 
                            data-toggle="tab">Home</a>
                            </li>
                            <li role="presentation">
                                <a href="#smartphone" aria-controls="smartphone" role="tab" 
                                data-toggle="tab">PSD</a>
                            </li>
                            <li role="presentation">
                            <a href="#accessories" aria-controls="accessories" role="tab" 
                            data-toggle="tab">Icons</a>
                            </li>
                        </ul>
                    </div>
                </div>-->
                <!--end tab button -->
                <!--start tab content-->
                <div class="row">
                    <div class="tab-top-contain tab-content">
                        <!--SINGLE TAB-->
                        <div  class="tab-pane active" id="laptop">
                            <div class="tab-carosel-h2-start">
                               <!--SINGLE CAROUSEL-->
                                <div class="col-md-12">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/icons/index4.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/icons/index4.jpg" alt="" style="padding-top:10px"></a>     
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/psd/images1.jpg" alt="" style="height:200px;width:100%"></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/psd/images1.jpg" alt="" style="padding:10px"></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Etiam molestie</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product"> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/download (1).jpg" alt="" style="height:200px;width:100%"></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/download (1).jpg" alt="" style="height:200px;width:100%;padding:10px"></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean sagittis</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$560</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/download (5).jpg" alt="" style="height:200px;width:100%"></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/download (5).jpg" alt="" style="height:200px;width:100%;padding:10px"></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Congue lectus</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/images (3).jpg" alt="" style="height:200px;width:100%"></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/images (3).jpg" alt="" style="height:200px;width:100%;padding:10px"></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>

                        <div  class="tab-pane" id="smartphone">
                            <div class="tab-carosel-h2-start">
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-6a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-9b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Congue lectus</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-14a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-14b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-1a.jpg" alt=""></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-3a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-3b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-4a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-4b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Etiam molestie</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product"> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-5a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-5b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean sagittis</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$560</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product"> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-8a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-8b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean sagittis</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$560</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                            </div>    
                        </div>
                        <!--SINGLE TAB-->
                        <div  class="tab-pane" id="accessories">
                            <div class="tab-carosel-h2-start">
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-7a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-7b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-12a.jpg" alt=""></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-3a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-3b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-6a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-9b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Congue lectus</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span>
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-1a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-1b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$60</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-15a.jpg" alt=""></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product"> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-3a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-3b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean sagittis</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$560</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <!--END TOP TAB AREA -->    
        </div> 
    </section>
    
    <!--CATEGORY ONE AREA START-->
    <section class="category-area-one control-car mar-bottom">
        <div class="container-fluid">
            <div class="row">
                <div class="cat-area-heading">
                    <h4><strong>PSD</strong></h4>
                </div>
                
                <div class="col-md-9 col-lg-10 no-padding">
                    
                    <div class="col-md-12 col-lg-9 no-padding">
                        <div class="cat-carousel-area">
                            <div class="cat-carousel">
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/images.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/images.jpg" alt="" style="padding:10px"></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                       
									   <div class="show-img">
                                               <a href="#"><img src="img/images (2).jpg" alt="" style="height:220px"></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/download (5).jpg" alt="" style="height:220px"></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/download (5).jpg" alt="" style="height:220px;padding:10px" ></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                               
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--CATEGORY ONE AREA END-->
    <!--CATEGORY TWO AREA START-->
    <section class="category-area-two control-car mar-bottom">
        <div class="container-fluid">
            <div class="row">
                <div class="cat-area-heading">
                    <h4><strong>Vector</h4>
                </div>
                
                <div class="col-md-9 col-lg-10 no-padding">
                    
                    <div class="col-md-12 col-lg-9 no-padding">
                        <div class="cat-carousel-area">
                            <div class="cat-carousel">
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/download (1).png" alt="" style="height:230px"></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/download (1).png" alt="" style="height:230px;padding:10px"></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-8a.jpg" alt=""></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-9a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-9b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-10a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-10b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Congue lectus</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span>
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-12a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-12b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$60</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-13a.jpg" alt=""></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--CATEGORY TWO AREA END-->
    <!--CATEGORY THREE AREA START-->
    <section class="category-area-three control-car mar-bottom">
        <div class="container-fluid">
            <div class="row">
                <div class="cat-area-heading">
                    <h4><strong>Icons</strong></h4>
                </div>
                
                <div class="col-md-9 col-lg-10 no-padding">
                   
                   
                    <div class="col-md-12 col-lg-9 no-padding">
                        <div class="cat-carousel-area">
                            <div class="cat-carousel">
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-6a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-9b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Congue lectus</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-14a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-14b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-1a.jpg" alt=""></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-3a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-3b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-4a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-4b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Etiam molestie</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product"> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-5a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-5b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean sagittis</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$560</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--CATEGORY THREE AREA END-->
    <!--CATEGORY FOUR AREA START-->
    <section class="category-area-four control-car mar-bottom">
        <div class="container-fluid">
            <div class="row">
                <div class="cat-area-heading">
                    <h4><strong>Logos</strong></h4>
                </div>
               
                <div class="col-md-9 col-lg-10 no-padding">

                    <div class="col-md-12 col-lg-9 no-padding">
                        <div class="cat-carousel-area">
                            <div class="cat-carousel">
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-7a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-7b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-12a.jpg" alt=""></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-3a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-3b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-6a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-9b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Congue lectus</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span>
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-1a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-1b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$60</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-15a.jpg" alt=""></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--CATEGORY FOUR AREA END-->
    <!--CATEGORY FIVE AREA START-->
    <section class="category-area-five control-car mar-bottom">
        <div class="container-fluid">
            <div class="row">
                <div class="cat-area-heading">
                    <h4><strong>Stock Photos</strong></h4>
                </div>
                
                <div class="col-md-9 col-lg-10 no-padding">
                    
                    <div class="col-md-12 col-lg-9 no-padding">
                        <div class="cat-carousel-area">
                            <div class="cat-carousel">
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-6a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-3b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Malesuada fames</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-5a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-5b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-9a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-9b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Congue lectus</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span>
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-16a.jpg" alt=""></a> 
                                            </div>
                                            <div class="hide-img">
                                                <a href="#"><img src="img/products/prod-16b.jpg" alt=""></a>    
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Condimentum posuere</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$60</span>        
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-15a.jpg" alt=""></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                                <!--SINGLE CAROUSEL-->
                                <div class="col-md-12 ">
                                    <div class="single-product">
                                        <span class="sale-on">sale</span> 
                                        <span >
                                            <a class="quick-view"  title="Quick View" data-target="#productModal" data-toggle="modal"  href="#"><i class="fa fa-external-link"></i></a>
                                        </span> 
                                        <div class="product-image">
                                            <div class="show-img">
                                               <a href="#"><img src="img/products/prod-11a.jpg" alt=""></a> 
                                            </div>
                                        </div>
                                        <div class="prod-info">
                                            <h2 class="pro-name">
                                                <a href="#">Aenean eu tristique</a>
                                            </h2>
                                            <div class="rating">
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star"></i>
                                                <i class="fa fa-star-half-o"></i>
                                            </div>
                                            <div class="price-box">
                                                <div class="price">
                                                    <span>$50</span>        
                                                </div>
                                                <div class="old-price">
                                                    <span>$55</span>    
                                                </div>
                                            </div>
                                            <div class="actions">
                                                <span class="add-to-cart">
                                                    <a href="#" data-toggle="tooltip" title="Add to cart">
                                                        <i class="fa fa-shopping-cart"></i>
                                                        <span>add to cart</span>
                                                    </a>
                                                </span>
                                                <span class="new-pro-wish">
                                                    <a href="#" data-toggle="tooltip" title="Add to wishlist"><i class="fa fa-heart-o"></i>
    </a>
                                                </span>
                                                <span class="new-pro-compaire">
                                                    <a href="#" data-toggle="tooltip" title="Add to compare"><i class="fa fa-bar-chart"></i>
    </a>
                                                </span>
                                            </div>
                                        </div>
                                    </div>   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--CATEGORY FIVE AREA END-->
   
    <!--FOOTER AREA START-->
    <section class="footer-area">
        <div class="container-fluid">
            <div class="row">
                <div class="footer-top">
                   <div class="col-sm-6 col-md-2 border-right">
                        <div class="single-footer">
                            <h3 class="footer-top-heading">About GoodPik</h3>
                            <div class="footer-list">
                                <ul>
                                    <li class="footer-list-item"><a href="#">Goodpik History</a></li>
                                    <li class="footer-list-item" ><a href="#">Social Responsibility</a></li>
                                    <li class="footer-list-item"><a href="#">Investor Relations</a></li>
                                    <li class="footer-list-item" ><a href="#">Healijy on Papers</a></li>
                                </ul>
                            </div>							
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-2 border-right">
                        <div class="single-footer">
                            <h3 class="footer-top-heading">Information</h3>
                            <div class="footer-list">
                                <ul>
                                    <li class="footer-list-item"><a href="#">our blog</a></li>
                                    <li class="footer-list-item" ><a href="#">about our Product</a></li>
                                    <li class="footer-list-item"><a href="#">secure shopping</a></li>
                                    <li class="footer-list-item" ><a href="#">privacy policy</a></li>
                                    <li class="footer-list-item" ><a href="#">delivery information</a></li>
                                </ul>
                            </div>							
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-2 border-right ">
                        <div class="single-footer">
                            <h3 class="footer-top-heading">my account</h3>
                            <div class="footer-list">
                                <ul>
                                    <li class="footer-list-item"><a href="#">my account</a></li>
                                    <li class="footer-list-item" ><a href="#">shopping cart</a></li>
                                    <li class="footer-list-item"><a href="#">custom link</a></li>
                                </ul>
                            </div>							
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-3 border-right">
                        <div class="contact-us">
                            <ul>
                                <li><i class="fa fa-phone"></i>PHONE: 9904182831</li>
                                <li><i class="fa fa-truck"></i>FREE SHIPPING ANYWHERE WORLDWIDE</li>
                                <li><i class="fa fa-unlock-alt"></i>100% SECURE FOR PAYMENT</li>
                                <li><i class="fa fa-headphones"></i>24/24 ONLINE SUPPORT CUSTOME</li>
                            </ul>						
                        </div>   
                    </div>
                    <div class="col-sm-6 col-md-3">
                        <div class="news-letter">
                            <h3 class="footer-top-heading">send newsletter</h3>
                            <div class="newsletter-wrapper">
                                <div class="subscribe-inner">
                                    <input type="text" id="subscribe_email">
                                    <a class="sub-button" href="#">subscribe</a>
                                </div>
                            </div> 
                            <div class="payment">
                                <img src="img/payment.png" alt="">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="footer-bottom">
                    <div class="col-xs-12 col-sm-8 col-md-8 col-lg-6">
                        

            </div>
        </div>
    </section>
    <!--FOOTER AREA END-->
    <!-- QUICKVIEW PRODUCT -->
	<div id="quickview-wrapper">
		<!-- Modal -->
		<div class="modal fade" id="productModal" tabindex="-1" role="dialog">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					</div>
					<div class="modal-body">
						<div class="modal-product">
							<div class="product-images">
								<div class="main-image images">
									<img alt="" src="img/products/prod-10b.jpg">
								</div>
							</div><!-- .product-images -->
							
							<div class="product-info">
								<h1>Diam quis cursus</h1>
								<div class="price-box">
									<p class="price"><span class="special-price"><span class="amount">$132.00</span></span></p>
								</div>
								<a href="shop.html" class="see-all">See all features</a>
								<div class="quick-add-to-cart">
									<form method="post" class="cart-qv">
										<div class="numbers-row">
											<input type="number" id="french-hens" value="3">
										</div>
										<button class="single_add_to_cart_button" type="submit">Add to cart</button>
									</form>
								</div>
								<div class="quick-desc">
									Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam fringilla augue nec est tristique auctor. Donec non est at libero vulputate rutrum. Morbi ornare lectus quis justo gravida semper. Nulla tellus mi, vulputate adipiscing cursus eu, suscipit id nulla.
								</div>
								<div class="social-sharing">
									<div class="widget widget_socialsharing_widget">
										<h3 class="widget-title-modal">Share this product</h3>
										<ul class="social-icons">
											<li><a target="_blank" title="Facebook" href="#" class="facebook social-icon"><i class="fa fa-facebook"></i></a></li>
											<li><a target="_blank" title="Twitter" href="#" class="twitter social-icon"><i class="fa fa-twitter"></i></a></li>
											<li><a target="_blank" title="Pinterest" href="#" class="pinterest social-icon"><i class="fa fa-pinterest"></i></a></li>
											<li><a target="_blank" title="Google +" href="#" class="gplus social-icon"><i class="fa fa-google-plus"></i></a></li>
											<li><a target="_blank" title="LinkedIn" href="#" class="linkedin social-icon"><i class="fa fa-linkedin"></i></a></li>
										</ul>
									</div>
								</div>
							</div><!-- .product-info -->
						</div><!-- .modal-product -->
					</div><!-- .modal-body -->
				</div><!-- .modal-content -->
			</div><!-- .modal-dialog -->
		</div>
		<!-- END Modal -->
	</div>
	<!-- END QUICKVIEW PRODUCT -->
    
    
    <!-- JS -->
        
 		<!-- jquery-1.11.3.min js
		============================================ -->         
        <script src="js/vendor/jquery-1.11.3.min.js"></script>
        
        <!-- price-slider js -->
        <script src="js/price-slider.js"></script>
        
        <!-- bootstrap js
		============================================ -->         
        <script src="js/bootstrap.min.js"></script>
        
		<!-- nevo slider js 
		============================================ --> 
		<script src="js/jquery.nivo.slider.pack.js"></script>
        
   		<!-- owl.carousel.min js
		============================================ -->       
        <script src="js/owl.carousel.min.js"></script>
        
        <!-- count down js 
        ============================================ -->  
        <script src="js/jquery.countdown.min.js" type="text/javascript"></script>
        
        <!--zoom plugin
        ============================================ --> 
        <script src='js/jquery.elevatezoom.js'></script>
        
   		<!-- wow js
		============================================ -->       
        <script src="js/wow.js"></script>
        
        <!--Mobile Menu Js
        ============================================ --> 
        <script src="js/jquery.meanmenu.js"></script>
        
        <!-- jquery.fancybox.pack js -->
		<script src="js/fancybox/jquery.fancybox.pack.js"></script>	
        
        <!-- jquery.scrollUp js 
        ============================================ -->
        <script src="js/jquery.scrollUp.min.js"></script>
        
        <!-- mixit-up js
		============================================ -->       
        <script src="js/jquery.mixitup.min.js"></script>
        
   		<!-- plugins js
		============================================ -->         
        <script src="js/plugins.js"></script>
        
   		<!-- main js
		============================================ -->           
        <script src="js/main.js"></script>
    </body>
</html>
